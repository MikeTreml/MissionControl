...
export function languageToDeviconSlug(raw) {
    const trimmed = raw.trim().toLowerCase();
    if (!trimmed)
        return null;
    const key = trimmed.replace(/\s+/g, "");
    if (LANG_TO_DEVICON[key])
        return LANG_TO_DEVICON[key];
    const alnum = key.replace(/[^a-z0-9+#.]/g, "");
    if (LANG_TO_DEVICON[alnum])
        return LANG_TO_DEVICON[alnum];
    const dashed = trimmed
        .replace(/\s+/g, "-")
        .replace(/[^a-z0-9-+#.]/g, "");
    if (dashed && LANG_TO_DEVICON[dashed])
        return LANG_TO_DEVICON[dashed];
    const dashedCompact = dashed.replace(/-/g, "");
    if (dashedCompact && LANG_TO_DEVICON[dashedCompact])
        return LANG_TO_DEVICON[dashedCompact];
...
export function deviconSvgUrl(slug, variant = "original") {
