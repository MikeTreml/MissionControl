/**
 * Derive the lane-history chips that render under the Running + Done
 * lanes (canvas: "Recent runs · last 24h" and "Shipped · last 7d").
 *
 * Running strip: last `run-ended` events within 24h, mapped to chip
 * state via the event's `exit` field (completed → success, failed →
 * failed, paused/cancelled → paused).
 *
 * Done strip: tasks at boardStage="Done" updated within 7 days. State
 * is always "success" — the lane name implies it.
 *
 * Both strips cap at 8 chips so the canvas's horizontal scroll behavior
 * is the visible affordance for "more".
 */
import type { TaskEvent } from "../../../shared/models";
import type { UiTask } from "../hooks/useTasks";

export type ChipState = "success" | "failed" | "paused";
...
export function buildRunningHistory(
...
export function buildShippedHistory(tasks: UiTask[], now = Date.now()): LaneChip[] {
  const cutoff = now - 7 * DAY_MS;
  return tasks
    .filter((t) => t.boardStage === "Done")
    .map((t) => {
      const ts = new Date(t.updatedAt).getTime();
      return { taskId: t.id, state: "success" as ChipState, whenMs: now - ts, whenLabel: relativeShort(now - ts), ts };
    })
