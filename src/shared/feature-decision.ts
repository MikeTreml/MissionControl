export type ScopePolicy = "strict" | "balanced" | "flexible" | "experimental";
export type ImpactPolicy = "low" | "medium" | "high";
export type RiskPolicy = "low" | "medium" | "high";
export type AutoProceedMode = "off" | "suggest" | "auto";

export type ProjectDecisionPolicy = {
  scope: ScopePolicy;
  impact: ImpactPolicy;
  risk: RiskPolicy;
  autoProceedMode?: AutoProceedMode;
};

export type FeatureDecisionScore = {
  scope: number;
  impact: number;
  risk: number;
};

export type FeatureDecisionResult = {
  decision: "no" | "defer" | "guard" | "proceed";
  reason: string;
  score: FeatureDecisionScore;
...
export const DEFAULT_PROJECT_DECISION_POLICY: ProjectDecisionPolicy = {
  scope: "balanced",
  impact: "medium",
  risk: "medium",
...
const SCOPE_THRESHOLDS: Record<ScopePolicy, number> = {
  strict: 85,
  balanced: 70,
  flexible: 55,
  experimental: 40,
...
const IMPACT_THRESHOLDS: Record<ImpactPolicy, number> = {
  low: 50,
  medium: 70,
  high: 85,
};

const RISK_THRESHOLDS: Record<RiskPolicy, number> = {
  low: 40,
  medium: 60,
  high: 85,
};
...
export function scoreBand(value: number): "very-low" | "low" | "medium" | "high" | "very-high" {
  const v = clampPercent(value);
  if (v < 40) return "very-low";
  if (v < 60) return "low";
...
export function evaluateFeatureDecision(
  score: FeatureDecisionScore,
  policy: Partial<ProjectDecisionPolicy> = {},
): FeatureDecisionResult {
  const resolvedPolicy: ProjectDecisionPolicy = {
    ...DEFAULT_PROJECT_DECISION_POLICY,
    ...policy,
  };

  const normalizedScore: FeatureDecisionScore = {
